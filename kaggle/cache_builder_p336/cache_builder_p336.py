...
G = int(os.environ.get("SLOT_G", "4"))
T = int(os.environ.get("SLOT_T", "1"))
CROP_MM = os.environ.get("SLOT_CROP_MM", "140")
TRIM = os.environ.get("SLOT_TRIM", "0.15")
ZOOM_MM = os.environ.get("SLOT_ZOOM_MM", "")          # e.g. "100"
ZOOM_SLOTS = os.environ.get("SLOT_ZOOM_SLOTS", "")    # e.g. "SAG_FS,COR_FS"
...
OUT = f"/kaggle/working/slots_P{P}" + ("" if TRIM == "0.15" else f"_t{int(float(TRIM)*100)}") + ("" if (G, T) == (3, 3) else f"_g{G}t{T}") + ("" if not ZOOM_MM else f"_z{ZOOM_MM}")
...
def main():
    ...
    if ZOOM_MM and ZOOM_SLOTS:
        common += ["--zoom-mm", ZOOM_MM, "--zoom-slots", ZOOM_SLOTS]
    if LIMIT:
        common += ["--limit", LIMIT]
