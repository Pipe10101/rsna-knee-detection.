"""Kaggle CPU kernel: HALF A of the G=20 coverage cache (224 px, 20 single-slice anchors/slot).

Full cache = 4,407 x 6 x 20 x 224 x 224 bytes = 23.8 GB > the 20 GB working cap, so it is built as
two halves over the SAME sorted uid list (part A: first 2204 studies; part B: --skip 2204) and merged
locally with scripts/merge_slot_cache.py (bitwise-verified against a direct build).
Output: /kaggle/working/slots_P224_g20t1_partA/ (~11.9 GB) + the 3-study test split (part A).
"""
import json
import os
import shutil
...
G = int(os.environ.get("SLOT_G", "20"))
...
SKIP = os.environ.get("SLOT_SKIP", "0")
LIMIT_N = os.environ.get("SLOT_LIMIT_N", "2204")   # "" = to the end
PART = os.environ.get("SLOT_PART", "A")
