...
def main():
    ...
    #   accuracy entry   : SK_CKPT_GLOB=*fold_*_best.pt         (both 5-fold sets; student_* excluded)
    ...
    # 2026-09-16: the ACCURACY entry is TWO 5-fold sets rank-averaged per finding -- the central-band
    # folds (plain fold_k_best.pt, trim 0.35) and the coverage folds (g10t1_fold_k_best.pt, trim 0.15):
    # pooled OOF 0.8843 vs 0.8836 / 0.8812 alone.  The glob therefore takes every *fold_* checkpoint
    # and the full-fit student (efficiency entry only, no OOF) is dropped unless asked for by name.
    ckpt_glob = os.environ.get("SK_CKPT_GLOB", "*fold_*_best.pt")
    ckpts = sorted(glob.glob(os.path.join(ckpt_dir, "**", ckpt_glob), recursive=True))
    if not ckpt_glob.startswith("student"):
        ckpts = [c for c in ckpts if not os.path.basename(c).startswith("student_")]
    print(f"checkpoint glob: {ckpt_glob} -> {[os.path.basename(c) for c in ckpts]}", flush=True)
    if not ckpts:
    ...
    if cpu_mode:
        ...
        tta_on = os.environ.get("SK_TTA", "1") == "1"
