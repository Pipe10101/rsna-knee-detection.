"""Kaggle kernel: build the JOINT-CENTRED zoom slot cache on the g10t1 coverage layout (step 6).

Layout: 6 base slots + 2 zoom slots (SAG_FS_Z, COR_FS_Z, 100 mm crops centred on the located
tibiofemoral joint, src/slots.locate_joint) x 10 single-slice anchors x 224 px.
Size: 4,407 x 8 x 10 x 1 x 224 x 224 bytes = 17.7 GB (under the 20 GB working cap).
...
ZOOM_MM = os.environ.get("SLOT_ZOOM_MM", "100")          # e.g. "100"
ZOOM_SLOTS = os.environ.get("SLOT_ZOOM_SLOTS", "SAG_FS,COR_FS")    # e.g. "SAG_FS,COR_FS"
ZOOM_CENTER = os.environ.get("SLOT_ZOOM_CENTER", "joint")        # "image" | "joint" (scripts/build_slot_cache.py --zoom-center)
...
OUT = f"/kaggle/working/slots_P{P}" + ("" if TRIM == "0.15" else f"_t{int(float(TRIM)*100)}") + ("" if (G, T) == (3, 3) else f"_g{G}t{T}") + ("" if not ZOOM_MM else f"_z{ZOOM_MM}") + ("j" if ZOOM_CENTER == "joint" else "")
...
def main():
    ...
    report = {"elapsed_min": round((time.time() - T0) / 60, 1), "P": P, "G": G, "T": T,
              "crop_mm": CROP_MM, "zoom_mm": ZOOM_MM, "zoom_slots": ZOOM_SLOTS, "zoom_center": ZOOM_CENTER, "out": OUT}
